...
public class SqlRunnerUtil {
    ...
    private String delimiter = SqlRunnerUtil.DEFAULT_DELIMITER;
    private final PrintWriter out, err;

    public SqlRunnerUtil(final Connection connection, final PrintWriter out, final PrintWriter err, final boolean autoCommit, final boolean stopOnError) {
        if (connection == null) {
        ...
        try {
            ...
            while ((line = lineReader.readLine()) != null) {
                ...
                if (trimmedLine.startsWith("--") || trimmedLine.startsWith("//") || trimmedLine.startsWith("#")) {
                    ...
                    final Pattern pattern = Pattern.compile(SqlRunnerUtil.DELIMITER_LINE_REGEX);
                    final Matcher matcher = pattern.matcher(trimmedLine);
                    if (matcher.matches()) {
                        delimiter = trimmedLine.split(SqlRunnerUtil.DELIMITER_LINE_SPLIT_REGEX)[1].trim();
